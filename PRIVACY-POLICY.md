...
DXF and DWG drawings are processed locally in your browser and are not uploaded
to any server or cloud service.

The Extension can send limited, anonymous product analytics events (for example:
file open success/failure and feature usage) so we can improve reliability and
prioritize features. Analytics never includes drawing content, file names, or
personal identity fields. You can disable analytics in the popup settings.
...
The Extension does not intentionally collect or send IP addresses, and it does
not use fingerprinting techniques to identify users.
...
The Extension uses PostHog for anonymous product analytics. Analytics is used to
understand reliability and feature adoption (for example, whether a drawing
opened successfully and which viewer tools are used).

What analytics may include:

- Anonymous random identifier generated by the Extension
- Extension version and UI locale
- Event names and non-sensitive event properties (for example, file type
  `dxf|dwg`, size bucket, action success/failure)

What analytics never includes:

- Drawing file names
- Drawing file content, geometry, or text
- Personal profile data (name, email, phone, address)

Analytics can be disabled from the Extension popup ("anonymous analytics"
toggle). Disabling analytics stops sending new analytics events.
...
The Extension does not use advertising networks and does not sell personal data.
It uses the following external services:

- **PostHog** for anonymous product analytics events sent from the Extension
- **Umami** analytics scripts on the optional welcome/uninstall pages hosted on
  GitHub Pages
